...
def get_run_info(dirname):
    ...
    try:
        total_time_str = (
            re.search(r"Elapased for Inference: ([\d:.]+)", log_content)
            .group(1)
            .removesuffix(".")
        )
...
def get_group_info(dirname):
    ...
    df["total_applied_count"] = df["lemma_applied_count"].apply(
...
def process_target_paral(model, target_paral, num_layers_list):
    ...
    for num_layers in num_layers_list:  
        ...
        data_list.append((model_rename[model], num_layers, target_paral, num_nodes, num_edges, total_time, saturation, extraction, total_applied_count))
    return data_list
...
def plot_performance():
    ...
    all_df = pd.DataFrame(
        data_list,
        columns=["Model", "Num Layers", "TP-size", "#of Nodes", "#of Edges", "Total Time", "Saturation Time", "Extraction Time", "Total Applied Count"],
    )
...
def plot_scalability():
    for model_idx, model in enumerate(["gpt", "aws_llama"]):
        ...
        with mp.Pool(processes=len(target_paral_list)) as pool:
            data_list = list(chain(*pool.starmap(process_target_paral, [(model, target_paral, num_layers_list) for target_paral in target_paral_list])))
        all_df = pd.DataFrame(
            data_list,
            columns=["Model", "Num Layers", "TP-size", "#of Nodes", "#of Edges", "Total Time", "Saturation Time", "Extraction Time", "Total Applied Count"],
        )
...
def plot_heatmap():
    ...
    for paral in [2, 4, 8]:
        lemma_applied_count_list.append((f"GPT({paral})", get_lemma_applied_count(f"precondition.gpt.fw.g0.paral1_layer1-paral{paral}_layer1.greedy")))
    for paral in [4]:
        lemma_applied_count_list.append((f"Qwen2({paral})", get_lemma_applied_count(f"precondition.qwen2.fw.g0.paral1_layer1-paral{paral}_layer1.greedy")))
    for paral in [4]:
        lemma_applied_count_list.append((f"Llama-3({paral})", get_lemma_applied_count(f"precondition.aws_llama.fw.g0.tp1_layer1-tp{paral}_layer1.greedy")))
    ...
    ax = fig.gca()
    sns.heatmap(unamed_df, ax=ax, linewidths=1, cmap="Oranges", mask=unamed_df.isnull(), cbar_kws=dict(orientation='vertical', pad=0.01))
