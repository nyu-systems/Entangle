...
from entangle.utils.print_utils import get_global_logger
...
def pgraph_to_sgraph(
    ...
    for pnode_idx, pnode in enumerate(tqdm(pgraph.nodes, leave=False)):
        ...
        try:
            ...
            LOGGER.error(f"Error when converting {pnode} to SExpr. {pnode=}\nop_mapping={op_mapping!s}")
            raise e
    ...
    assert output_pnode is not None, "There should be a output node in the end of pgraph."
    LOGGER.info(f"There are still {len(unreferenced_sexpr)} unreferenced nodes.")
    LOGGER.info(unreferenced_sexpr)

    # Some sym int outputs are just ignored.
    # 1. starting with _tensor_constant
    # 2. Sym Int with meta["val"] like "Sym(xxx > 0)"
    outputs = [pnode_to_sexpr[pnode] for pnode in output_pnode.args[0] if pnode is not None and not pnode.is_sym_scalar()]
    LOGGER.info(f"Using outputs: {outputs=}")
    if filter_output is not None:
        outputs_or_names = filter_output(outputs)
        name_to_sexpr = {sexpr.name: sexpr for sexpr in pnode_to_sexpr.values() if not sexpr.op.scalar}
        outputs = [
